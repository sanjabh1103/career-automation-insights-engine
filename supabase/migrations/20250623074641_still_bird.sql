...
-- Create policies for ai_task_assessments (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'ai_task_assessments' 
        AND policyname = 'Users can create their own task assessments'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'ai_task_assessments' 
        AND policyname = 'Users can view their own task assessments'
    ) THEN
...
-- Create policies for ai_skill_recommendations (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'ai_skill_recommendations' 
        AND policyname = 'Anyone can view skill recommendations'
    ) THEN
...
-- Create policies for ai_reskilling_resources (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'ai_reskilling_resources' 
        AND policyname = 'Anyone can view reskilling resources'
    ) THEN
